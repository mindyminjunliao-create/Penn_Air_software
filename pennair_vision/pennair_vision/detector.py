...
def detect_objects_rgb_and_noise(
    ...
    for cnt in contours:
        ...
        if M["m00"] != 0:
            ...
            cY = int(M["m01"] / M["m00"])
            centers.append((cX, cY))
            shape_contours.append(hull)  # Keep contour in sync with its center's index
    ...
    return output_frame, centers, shape_contours, clustered_mask, fg_color_mask_closed, fg_noise_mask_dilated
